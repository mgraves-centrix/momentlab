...
|---|---|---|---|---|
| **Cloud Run Web & API Service** | GCP Cloud Run | `momentlab-web` (Port 8080) | **READY** | Reachable at `https://momentlab.ai` and `https://www.momentlab.ai` via Cloud Run domain mappings (origin URL: `https://momentlab-web-qa24oxtrrq-uc.a.run.app`; `min-instances=1` active for judging week to eliminate cold-start latency; revert to `--min-instances=0` post-judging to control hosting cost) |
