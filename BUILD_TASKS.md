...
| P0.4 | Delete frontend facade, wire real health/timeline, unify numbers | DONE | ✅ | Total respondents query: `SELECT count(DISTINCT session_id) FROM momentlab.audience_events`. Hypothesis & Confidence: From persisted Firestore hypothesis synthesized by agent. Cohorts & Lift: SIMULATED (derived from agent forecast). All hardcoded values removed. | |
| P1.1 | Complete Evidence page (match 05-moment-evidence.png) | DONE | ✅ | Evidence chips mapped to `hypothesis.evidenceRecords`. | |
