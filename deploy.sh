...
gcloud run deploy "${SERVICE_NAME}" \
    ...
    --memory=2Gi \
    --cpu=2 \
    --set-env-vars="GIT_SHA=${GIT_SHA},GCP_PROJECT_ID=${PROJECT_ID},GCP_LOCATION=${REGION},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${REGION},GOOGLE_GENAI_USE_VERTEXAI=true,CLICKHOUSE_HOST=${CH_HOST},CLICKHOUSE_PORT=${CH_PORT},CLICKHOUSE_USER=${CH_USER},CLICKHOUSE_DB=${CH_DB},CLICKHOUSE_DATABASE=${CH_DB},CLICKHOUSE_SECURE=${CH_SECURE},CLICKHOUSE_WRITER_USER=${CH_WRITER_USER},CLICKHOUSE_MCP_USER=${CH_MCP_USER}" \
