...
gcloud run deploy "${SERVICE_NAME}" \
    ...
    --port=8080 \
    --set-env-vars="GCP_PROJECT_ID=${PROJECT_ID},GCP_LOCATION=${REGION},CLICKHOUSE_HOST=${CH_HOST},CLICKHOUSE_PORT=${CH_PORT},CLICKHOUSE_USER=${CH_USER},CLICKHOUSE_PASSWORD=${CH_PASS},CLICKHOUSE_DB=${CH_DB}" \
    --project="${PROJECT_ID}"
